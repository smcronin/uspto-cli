...
func TestMain(m *testing.M) {
	...
	// Check for API key — prefer environment, fallback to project .env for tests.
	apiKeyValue = strings.TrimSpace(os.Getenv("USPTO_API_KEY"))
	if apiKeyValue == "" {
	...
	if hasAPIKey {
		// Ensure child test processes always receive the key via environment.
		_ = os.Setenv("USPTO_API_KEY", apiKeyValue)
	}
